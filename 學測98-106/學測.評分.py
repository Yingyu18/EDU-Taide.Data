...
def main():
    for year in os.listdir(directory):
        ...
        for vol in os.listdir(year_dir): 
            if '.DS_Store' in vol:
                continue
            # if "98_1-1" not in vol:
            #     continue
            #print(vol)
            with open(f"{year_dir}/{vol}", 'r', encoding='utf-8') as file:    
                作答=file.read()
            with open(f"學測98-106/題目/{year}.txt", 'r', encoding='utf-8') as file:    
                題目=file.read()
            # print(題目)
            # print(作答)
            ...
            '''AI 評分''' 
            rateContent = rate(題目, 作答)
            rateObject = rate_format(rateContent)  
            print(rateObject)
            
            # TODO: 評分、等第、卷號 寫進 csv 
            data = rateObject
            data['題目']=題目
            data['作答']=作答
            # print(data)
            ...
            # data['總分']
            vol = vol.replace('.txt', '')
            output_path = f'學測98-106/csv/{year}/{vol}' 
            os.makedirs(output_path, exist_ok=True)    
            # print(f"{output_path}/{vol}")
            output_csv = f"{output_path}/{vol}-原卷.csv"
